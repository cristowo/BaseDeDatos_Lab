...
<body>
    @include('barra.barra')
    <h1 class="centrado">Top 10</h1>

<table class="styled-table" id="tabla" style="max-height: 715px; display: inline-block;">
<thead>
    <tr>
      ...
      @foreach ($songs as $song)
        ...
        <?php $a = 0; ?>
            @foreach($usersonglikes as $usersonglike)
                ...
                <form class= "element-selector" action="{{action('UserSongLikeController@destroy', [$usersonglike->id, 4 , $user->id, $song->id])}}" method='POST'>
                    @method('PUT')
                    <input type="int" class="form-control" id="id" name="id" value="{{$usersonglike->id}}" hidden>
                    <input type="int" class="form-control" id="page" name="page" value="4" hidden>
            ...
            @endforeach
            @if($a != '1')
            <a class="btn btn-success" href='/song/updatelike/4/{{$user->id}}/{{session("id_login")}}/{{$song->id}}'>❤</a>
            @endif
        </td>
